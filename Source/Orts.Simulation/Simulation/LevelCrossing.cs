    ...
    public class LevelCrossings
    {
        ...
        void UpdateCrossings(Train train, float elapsedTime)
        {
            ...
            foreach (var crossing in TrackCrossingItems.Values.Where(ci => ci.CrossingGroup != null))
            {
                ...
                // Train is stopped.
                else if ((train is AITrain || train.TrainType == Train.TRAINTYPE.PLAYER) && Math.Abs(speedMpS) <= 0.1f && frontDist <= reqDist && (train.ReservedTrackLengthM <= 0 || frontDist < train.ReservedTrackLengthM) && rearDist <= minimumDist)
                {
